...
def get_device_data_range(device_id, date_from, date_to, resolution=None):
    ...
    device_type = select_device_type(device)
    result = device_type.get_data_range(
        session, device.id, date_from, date_to, resolution)
    session.close()
    return result
...
def get_device_action_history(device_id):
    ...
    history_data = session.query(models.UserDeviceHistory).filter_by(
...
def save_device_config(form, device, config_uuid):
    try:
        ...
        device_type = select_device_type(device)
        device_config = device_type.save_configuration(
            session, form, device.id, config_uuid)

        if (config_uuid is None):
            session.add(device_config)

        if (device_config.job_state != enums.ConfigJobState.Running.value):
            response = configure_device_jobs(
                device, device_config, device_config.is_active)
            if (not response):
                session.rollback()
            else:
                session.commit()
                flash('Konfigūracija "' + device_config.name +
                      '" išsaugota!', 'success')
        else:
            session.rollback()
    ...
    try:
        ...
        if (is_active):
            job = main_db_session.query(models.DeviceJobs).filter_by(
                config_uuid=device_config.uuid, device_id=main_db_device.id).first()
...
def stop_job(device, config_uuid):
    try:
        ...
        save_device_history(session=session, device=device, text="Rutininis darbas pagal konfigūraciją \"" +
                            device_config.name + "\" sustabdytas naudotojo iniciatyva!")
